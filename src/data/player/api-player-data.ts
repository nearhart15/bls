...
export async function apiPlayerListFetcher(): Promise<PlayerListEntry[]> {
    const response=await fetch(`${import.meta.env.BASE_URL}data/beer-league.json?ts=${Date.now()}`,{cache:"no-store"}); if(!response.ok)throw new Error(`API data returned ${response.status}.`); const data=await response.json() as ApiLeagueData; if(data.status!=="ready")throw new Error("API data is not ready yet.");
