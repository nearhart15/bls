...
function compareRows(a: DisplayRow, b: DisplayRow, key: SortKey, dir: SortDir): number {
    ...
    switch (key) {
        ...
        case "grade": cmp = rowRating(a) - rowRating(b); break;
        case "rank":
        default:
    ...
    return (
        <Card className="mb-0 h-100 bls-perf-card">
            <CardHeader className="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <span>{title}</span>
                {data && <Badge bg="secondary" pill>{sorted.length} bowlers</Badge>}
            </CardHeader>
            {!lockScope && (
                            ...
                            {sorted.map((p, idx) => {
                                ...
                                return (
                                    <tr key={p.id}>
                                        ...
                                        <td><Link to={`/player/${p.id}`} className="bls-link fw-semibold">{p.name}</Link></td>
                                        <td className="text-end fw-semibold tabular-nums">{p.average != null ? numberFormat.format(p.average) : "—"}</td>
