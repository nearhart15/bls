...
const AppearSortTh: FC<{
    label: string;
    sortKey: AppearSortKey;
    active: AppearSortKey;
    dir: SortDir;
    onSort: (k: AppearSortKey) => void;
    className?: string;
...
const AppearancesPanel: FC<AppearancesProps> = ({appearances}) => {
    ...
    const onSort = (key: AppearSortKey) => {
        if (key === sortKey) {
            ...
            setSortKey(key);
            setSortDir(
                key === "league" || key === "team" || key === "season" || key === "status"
                    ? "asc"
                    : "desc"
            );
        }
        ...
        withIdx.sort((x, y) => {
            ...
            switch (sortKey) {
                ...
                case "status":
                    ...
                    break;
                case "games":
                    cmp = (A.stats?.gameStats.count ?? -1) - (B.stats?.gameStats.count ?? -1);
                    break;
                case "avg":
                    cmp = (A.stats?.gameStats.average ?? -1) - (B.stats?.gameStats.average ?? -1);
                    break;
                case "hg":
                    cmp = (A.stats?.gameStats.max ?? -1) - (B.stats?.gameStats.max ?? -1);
                    break;
                case "idx":
                default:
                    cmp = x.i - y.i;
...
const SeasonSortTh: FC<{
    ...
    active: SeasonSortKey;
    dir: SortDir;
    onSort: (k: SeasonSortKey) => void;
    className?: string;
}> = ({label, sortKey, active, dir, onSort, className}) => {
    const isActive = active === sortKey;
    return (
...
const SeasonBreakdownTable: FC<{seasons: PlayerSeasonStats[]}> = ({seasons}) => {
    ...
    const sorted = useMemo(() => {
        ...
        list.sort((a, b) => {
            ...
            switch (sortKey) {
                case "leagues":
                    cmp = a.leagues - b.leagues;
                    break;
                case "games":
                    cmp = a.games - b.games;
                    break;
                case "average":
                    cmp = a.average - b.average;
                    break;
                case "pinfall":
