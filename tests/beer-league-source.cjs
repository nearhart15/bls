...
function findBeerLeague(dataSets) {
  const candidates = dataSets.flatMap(({ action, data }) => objects(data).map(item => ({ action, item, score: beerScore(item) })))
...
function chooseLeaguePage(item) {
  ...
  return urls.find(entry => /permalink|page|link|url/i.test(entry.key) && !/pdf|stand/i.test(entry.key))?.url
    ?? urls.find(entry => !/\.pdf(?:$|[?#])/i.test(entry.url))?.url
    ?? null;
}
...
function pdfUrlsFromHtml(html, baseUrl) {
  ...
  for (const match of matches) {
    ...
    try { urls.push(new URL(raw, baseUrl).href); } catch { /* ignore malformed candidates */ }
  }
  return [...new Set(urls)];
}
...
(async () => {
  ...
  console.log(`2. Discovered public data actions: ${actions.join(', ')}`);

  const dataSets = [];
  for (const action of actions) dataSets.push({ action, data: await publicData(action) });
  const league = findBeerLeague(dataSets);
  console.log(`3. Found Beer league via ${league.action}: ${JSON.stringify(league.item)}`);

  const leaguePageUrl = chooseLeaguePage(league.item);
  if (!leaguePageUrl) throw new Error('Beer league data did not include a league page URL.');
  console.log(`4. Following View League target: ${leaguePageUrl}`);
  ...
  console.log(`5. Found standings PDF: ${pdfUrl}`);

  const bytes = Buffer.from(await (await request(pdfUrl)).arrayBuffer());
  if (bytes.length < 5 || bytes.subarray(0, 5).toString('ascii') !== '%PDF-') throw new Error(`Standings URL did not return a PDF (${bytes.length} bytes).`);
  console.log(`6. PDF download verified (${bytes.length.toLocaleString()} bytes).`);
  console.log(JSON.stringify({ source: LEAGUES_URL, leaguePage: leaguePageUrl, standingsPdf: pdfUrl, bytes: bytes.length }, null, 2));
})().catch(error => {
